...
namespace WorkWithVocabularyTests
{
	...
	public class Test
	{
		...
		public void CorrectlyTransformSingleLine ()
		{
			...
			Stack<string> vocabulary = new Stack<string>(new[] { "текст" });

//			foreach(var vocab_word in vocabulary){
//				Console.WriteLine ("vocabword {0}", vocab_word);
//			}
////
...
//			Assert.IsTrue(input_line.Count() == 2);
		...
		{
			var wwv_ins = new wwv ();
			Stack<string> vocabulary = new Stack<string>(new[] {"текст", "в", "знак"});
//			foreach(var vocab_word in vocabulary){
//				Console.WriteLine ("vocabword {0}", vocab_word);
//			}
