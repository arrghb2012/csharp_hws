...
//				return this.VocabFile_lineCount;
			...
			for (var x = 0; x < words.Count(); x++) {
				...
				foreach (var delim in delimiterChars){
					...
					if (delim == words[x][words[x].Length - 1]){
						...
						if (vocabwords.ContainsKey(new_word)){
				...
				if (vocabwords.ContainsKey(words[x])){
					this.found_in_vocab = true;
					if (x == (words.Count() - 1)){
		...
		public void set_vocabulary( ConcurrentDictionary<string, int> in_vocabwords = null )
		{
...
//				vocabwords = System.IO.File.ReadAllLines (@"VocabFile.txt");
				...
				vocabwords = new ConcurrentDictionary<string, int>();
				foreach(var vocab_word in list)
				{
					int value = 0;
					vocabwords.TryAdd(vocab_word, value);
				}
...
//			vocabwords = System.IO.File.ReadAllLines(@"VocabFile.txt");
		...
		// словарь
		private ConcurrentDictionary<string, int> vocabwords;
	}
